 ...
 *
 * \author YR, TE, PC
 * \version V4.0 + CONDITION_INITIALE
 */
...
#define ASSERTION(f) \
      if (!(f))      \
            throw AssertionException(__FILE__, __LINE__, #f);
#define PRECONDITION(f) \
      if (!(f))         \
            throw PreconditionException(__FILE__, __LINE__, #f);
#define POSTCONDITION(f) \
      if (!(f))          \
            throw PostconditionException(__FILE__, __LINE__, #f);
#define INVARIANT(f) \
      if (!(f))      \
            throw InvariantException(__FILE__, __LINE__, #f);
...
#endif // --- ifndef CONTRATEXCEPTION_H